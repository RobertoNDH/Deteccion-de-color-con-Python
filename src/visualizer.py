...
class Visualizer:
    ...
    def draw(self, frame: np.ndarray, tracked_objects: list[TrackedObject], show_trajectory: bool = True) -> np.ndarray:
        for obj in tracked_objects:
            ...
            cv2.rectangle(frame, (x, label_y - lh - 4), (x + lw + 4, label_y + 2), color, cv2.FILLED)
            cv2.putText(frame, label, (x + 2, label_y - 2), _FONT, _FONT_SCALE, (255, 255, 255), 1, cv2.LINE_AA)
            ...
            if show_trajectory and len(obj.trajectory) > 1:
                ...
                for i in range(1, len(pts)):
                    ...
                    cv2.line(frame, pts[i - 1], pts[i], faded, thick, cv2.LINE_AA)

        return frame

    def draw_hud(
        self, frame: np.ndarray, counts_by_color: dict[str, int], paused: bool = False, source_name: str = ""
    ) -> np.ndarray:
        self._update_fps()
    ...
    def _build_hud_lines(self, counts: dict[str, int], paused: bool, source: str) -> list[str]:
    ...
    def _render_hud(frame: np.ndarray, lines: list[str]):
        ...
        overlay = frame.copy()
        cv2.rectangle(overlay, (0, 0), (panel_w, panel_h), _HUD_BG, cv2.FILLED)
        cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)

        for i, line in enumerate(lines):
            y = pad + (i + 1) * line_h - 4
            cv2.putText(frame, line, (pad, y), _FONT_SMALL, 0.45, _HUD_TEXT, 1, cv2.LINE_AA)

    def draw_tripwire(
        self, frame: np.ndarray, tripwire: tuple[tuple[int, int], tuple[int, int]], counts: dict[str, int]
    ) -> np.ndarray:
        p1, p2 = tripwire
